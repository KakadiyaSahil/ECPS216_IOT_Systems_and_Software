...
Assignment Name: Programming Assignment 4 2
*/
...
#define LDR_BAR_MIN 0
#define LDR_BAR_MAX 8
...
void setup()
{
  ...
  for (int thisLed = 0; thisLed < LDR_BAR_MAX ; thisLed++) 
  {
    pinMode(ledPinsBar[thisLed], OUTPUT);
    digitalWrite(ledPinsBar[thisLed], LOW);
  }
...
void RPI_IncomingPackets(String packet, ServerConfig *config)
{
  if (packet != NULL)
  {
    ...
    if (cmd != NULL)
    {
      ...
      {
        cmd = strtok(NULL, "@");
        serverConfig.Listening_port = cmd.toInt();
//        Serial.println("serverConfig.Listening_port : " + String(serverConfig.Listening_port));
...
void sendToRPi(void)
{
    ...
    udp.beginPacket(serverConfig.IP_address.c_str(), serverConfig.Listening_port);
    udp.print(message);
    udp.endPacket();
}
...
void readLDR(void)
{
  ...
  intensityIndication();
  checkandsetMaster();
  if ( (counter >= 5) && (deviceConfig[DEVICE_OWN_ID].isMaster ))
  {
    counter = 0;
    sendToRPi();
  }
...
void intensityIndication(void)
{
  ...
  analogWrite(LED_FLASH_PIN, 50);
   // map the LED array:
  int ledLevel = map(deviceConfig[DEVICE_OWN_ID].LDR_reading, LDR_RAW_MIN, LDR_RAW_MAX, LDR_BAR_MIN, LDR_BAR_MAX);
  Serial.println("LDR value level: " + String(deviceConfig[DEVICE_OWN_ID].LDR_reading) + " LED Bar Indication level: " + String(ledLevel) + " White LED Flashing Rate: " + String(flashingRateValue));  
  ...
  for (int thisLed = 0; thisLed < LDR_BAR_MAX; thisLed++) 
  {
    ...
    if (thisLed < ledLevel) 
    {
      digitalWrite(ledPinsBar[thisLed], HIGH);
    }
